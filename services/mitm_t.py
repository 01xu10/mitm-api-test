...
class Counter:
    ...
    def __init__(self, intercept_hosts: tuple, keywords: tuple, filter_url_type: list):
        ...
        self.reserved_prefixes = [
            "/141/20204/snscosdownload/SZ/reserved/",
            "/131/20204/snssvpdownload/SH/reserved/",
            "/aweme/100x100/aweme-avatar/",
            "/WeChatOfficial/Version_1.1.7/remote/resources/native/",
            "/WeChatOfficial/Version_1.1.8/remote/resources/native/"
        ]
    ...
    def is_reserved_path(self, path):
        # 检查路径是否以任何一个保留前缀开头
        return any(path.startswith(prefix) for prefix in self.reserved_prefixes)
    ...
    def response(self, flow: http.HTTPFlow):
        ...
        if (
            ...
            if self.is_reserved_path(path):
                if path not in self.url_path_set:
                    self.url_path_set.add(path)
                    data = {
                        str(flow.id):
                            {
                                "path": path,
