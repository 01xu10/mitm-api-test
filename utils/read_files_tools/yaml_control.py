...
class GetCaseData(GetYamlData):
    ...
    def get_yaml_case_data(self):
        ...
        # 正则处理yaml文件中的数据
        re_data = str(_yaml_data)
        return ast.literal_eval(re_data)


if __name__ == '__main__':
    package_info = GetYamlData(ensure_path_sep("\\resources\\package.yaml")).get_yaml_data()
    for k, v in package_info.items():
        v: dict= v
        print(v.get('cut_size'))
        print(type(v.get('cut_size')))
        print(type(tuple(v.get('cut_size'))))